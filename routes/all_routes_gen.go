...
// 映射所有组=>地址
func (r *Routes) genRoutesConfig() map[string]groupMap {
	return map[string]groupMap{
		"open": r.mergerRouteMap(
			r.AdminRoutes.GetOpenRoutes(),
		),
		"test": r.mergerRouteMap(
			r.AdminRoutes.GetTestRoutes(),
		),
	}
}

func (r *Routes) mergerRouteMap(ms ...groupMap) groupMap {
	got := make(groupMap)
	for _, m := range ms {
...
func (r *Routes) Load(engine *gin.Engine, config []GroupConfig) {
	allGroupRoute := r.genRoutesConfig()
	...
	for s, b := range isUserConfig {
		if b == false {
			log.Println("proto的路由【", s, "】未在kernel配置前缀、中间件等信息, 它的路由信息未能正常加载")
		}
	}
}

// 分组路由加载, 前缀, 中间件, 路由函数列表
func (r *Routes) MiddlewareRoutes(
	engine *gin.Engine,
	path string,
...
func (r *Routes) loadRoutes(engine *gin.RouterGroup, routes map[*api.Config]func(c *gin.Context)) {
